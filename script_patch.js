...
(function(){
 ...
 window.renderExamLecturesList = function() {
   ...
   // إحصاء العدد الفعلي لكل محاضرة داخل أسئلة الامتحان الحالي
   const counts = {};
   ...
   if (!entries.length) {
     listContainer.innerHTML = '<div style="color:#aaa;font-style:italic;text-align:center;">لا توجد محاضرات</div>';
     return;
   }
   ...
   entries.forEach(item => {
     html += `<div style="display:flex; justify-content:space-between; align-items:center; padding:8px 0; border-bottom:1px solid rgba(255,255,255,0.1);">
       <div style="flex:1; text-align:left; font-size:12px; margin-right: 12px; line-height: 1.4;">${escapeHtml(item.name)}</div>
       <div style="flex:0 0 auto; text-align:right; font-weight:bold; font-size:13px; min-width: 20px;">${item.questionCount}</div>
     </div>`;
   });
 ...
 window.toggleExamLecturesPopup = function(event) {
   ...
   if (!isShowing) {
     ...
     // SetTimeout لتأخير ربط الحدث، مما يمنع إغلاق القائمة فوراً
     setTimeout(() => {
 ...
 // Override renderExam safely without destroying HTML layout 
 renderExam = function(){
   if(!state.currentExam) return;
